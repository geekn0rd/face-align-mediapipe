...
def draw_landmarks_on_original(original_img, landmarks, crop_coords, rotation_matrix):
    ...
    for nx, ny in landmarks:
        # Convert normalized coordinates to cropped image coordinates
        ...
        # Convert to original image coordinates using inverse rotation
        point = np.array([[x_rot, y_rot, 1]]).T
        original_point = inv_rotation_matrix @ point

        x = int(original_point[0][0])
        y = int(original_point[1][0])
